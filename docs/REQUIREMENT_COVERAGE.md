...
| M9 | Complete bidding/doubling/scoring engine, three-container belief, Bid Head, checksum/version-pinned Cardplay continuation, information-set-conditioned MC pretraining, metric-gated curriculum and resumable joint trainer | Warm-start identity rejection plus explicit random-continuation MC-pretrain and one-game joint train/load/resume smoke; joint-checkpoint and fixed-bid complete Arena smokes | A strong pretrained Cardplay artifact, calibrated non-degenerate bidder and positive paired Gate C run pending |
