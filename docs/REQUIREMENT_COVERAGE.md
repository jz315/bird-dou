...
| M5 | Exact two-container CRF, NLL/marginals/sampler, calibration, data generation and frozen/joint Belief model | Conservation/brute-force tests; smoke NLL 4.6409 vs uniform 5.1511 | Policy non-regression and Belief-shuffle paired research run pending |
| M6 | Full-hand Teacher, privileged critic, Oracle Dropout, direct KD and information-set KD | Leakage/isomorphism/gradient and target-difference tests | Teacher-strength and direct-vs-IS-KD playing-strength ablations pending |
| M7 | Bounded multiprocess Actors, vectorized native envs, central ragged inference, replay, V-trace, Hybrid learner and fair-budget validator | Queue-full/crash/restart/shutdown, multi-Actor full games and numerical tests | Long-duration memory/deadlock soak and equal-budget three-mode run pending |
...
| M9 | Complete bidding/doubling/scoring engine, three-container belief, Bid Head, information-set-conditioned MC pretraining, metric-gated curriculum and resumable joint trainer | MC-pretrain + one-game joint train/load/resume smoke; joint-checkpoint and fixed-bid complete Arena smokes | Calibrated non-degenerate bidder and positive paired Gate C run pending |
