...
下列项尚未冻结具体值。R002 创建 RuleConfig v2 时必须显式表达；profile
...
R001 只验证规格和金标的完整性。它不声称 Rust 引擎已实现欢乐流程，也不以
JSON corpus 代替 R002–R010 的规则、序列化、replay、Web 和 differential
验收。任何实现提交都必须引用相应 case ID，并将其从“规格金标”提升为真实
引擎测试。
