...
玩家。队列从 caller 的下一席开始循环一圈，保留每个 `!declined` 席位；初始
candidate 到达自己的尾部位置时 **MUST** 自动跳过，不得制造“自己抢自己”的
动作或倍率。candidate 被别人抢走后，原 caller 在其尚未使用的尾部机会是否
可以反抢，只能由 `robbing.caller_can_reclaim` 显式配置决定；关闭该配置时该
尾部位置同样 **MUST** 自动跳过。
...
R005 实现显式 `CallStateV2`：合法初始动作严格只有 CallLandlord / PassCall；
首次 CallLandlord 立即关闭 Calling，保存 caller 并进入 R006 的 Robbing 边界；
PassCall 按座位循环、逐席记录 `acted` 和 `declined`。全员 Pass 时，若有首明牌者，
该席直接成为地主并进入 R007 的 BottomReveal 边界；若没有明牌者，完整 call state
与动作历史进入 all-pass summary，并在同一 match 开始下一次确定性 attempt。

R006 实现显式 `RobStateV2 { caller, candidate, eligible, acted, queue,
successful_rob_count, rob_factor }`。`eligible` 严格从 `!CallStateV2::declined`
派生，`queue.front()` 是唯一具有 `Rob / PassRob` 合法动作的席位。每个真实
决策均以事务方式写入 attempt action history；成功 Rob 更新 candidate、计数并
按配置的 `factor_per_successful_rob` 乘公共 `rob_factor`，PassRob 不改变二者。
队列耗尽后 Rust 自动记录 candidate 为地主并停止在 `BottomReveal`，且 replay 必须
复现自动附加的 `LandlordResolved` decision-event suffix。不能通过通用 action
接口或公开 landlord 记录接口绕过 Rob 队列。

R006 的边界止于已解析地主的 `BottomReveal`：它尚不转移底牌、不公开底牌、不处理
收底牌后明牌、加倍、出牌或结算；这些动作仍只能由后续 ticket 的 authoritative
状态机实现。
